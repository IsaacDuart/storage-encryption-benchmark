...
O objetivo do trabalho e comparar o desempenho de leitura e escrita em tres cenarios de armazenamento: uma pasta comum sem criptografia, um volume LUKS e um volume VeraCrypt. A comparacao busca evidenciar o impacto da criptografia no tempo de acesso, na vazao e no uso de CPU.

## Metodo utilizado

A execucao principal e feita por um script `.sh`, que usa `dd` com I/O direto para gravar e ler arquivos reais nos caminhos configurados. Os volumes LUKS e VeraCrypt devem estar previamente montados no sistema operacional. O benchmark nao simula a criptografia internamente: ele mede o comportamento observado ao acessar os pontos de montagem reais.
...
Espera-se que a pasta sem criptografia apresente menor tempo e maior vazao, pois nao ha custo de cifragem no caminho de E/S. Nos volumes LUKS e VeraCrypt, a criptografia tende a aumentar o uso de CPU e reduzir a vazao, principalmente em arquivos maiores ou em sistemas sem aceleracao eficiente para AES. A comparacao final deve ser feita com base no CSV e nos graficos gerados na maquina usada pela equipe.
